...
export class ShoonyaService implements OnModuleInit {
    ...
    async findAtmOption(symbol: string, triggerPrice: number, type: 'CE' | 'PE', preferITM: boolean = false): Promise<OptionContract | null> {
        ...
        const NSE_STOCK_STEPS: Record<string, number> = {
            ...
            'BRITANNIA': 20,
            'HDFCLIFE': 10,
            // Nirwana GANN_ANGLE universe additions (steps from Nirwana option_helper.py STRIKE_STEPS)
            'MUTHOOTFIN': 50, 'ASTRAL': 20, 'CHOLAFIN': 20, 'SUPREMEIND': 50,
            'COFORGE': 20, 'PRESTIGE': 20, 'TVSMOTOR': 20, 'PIDILITIND': 50,
            'CGPOWER': 10, 'ADANIGREEN': 20, 'MANKIND': 20, 'POLICYBZR': 20,
            'HDFCAMC': 20, 'ALKEM': 100, 'LUPIN': 10, 'GLENMARK': 20,
    ...
    async findTopItmOptionByVolume(symbol: string, triggerPrice: number, type: 'CE' | 'PE'): Promise<OptionContract | null> {
        ...
        const NSE_STOCK_STEPS: Record<string, number> = {
            ...
            'RELIANCE': 20, 'INFY': 10, 'TCS': 20, 'ADANIENT': 20, 'ADANIPORTS': 10,
            'M&M': 10, 'HDFCBANK': 10, 'ICICIBANK': 10, 'SBIN': 5, 'TATASTEEL': 2.5,
            'ITC': 2.5, 'DIXON': 50, 'MARUTI': 100, 'BAJFINANCE': 50, 'DRREDDY': 50,
            ...
            'CIPLA': 10, 'BHARTIARTL': 10, 'BAJAJFINSV': 20, 'KOTAKBANK': 20,
            'HCLTECH': 20, 'WIPRO': 5, 'ONGC': 5, 'NTPC': 5, 'COALINDIA': 5,
            'POWERGRID': 5, 'COLPAL': 50, 'GODREJCP': 10, 'BRITANNIA': 20, 'HDFCLIFE': 10,
            // Nirwana GANN_ANGLE universe (steps from Nirwana option_helper.py STRIKE_STEPS)
            'MUTHOOTFIN': 50, 'ASTRAL': 20, 'CHOLAFIN': 20, 'SUPREMEIND': 50,
            'COFORGE': 20, 'PRESTIGE': 20, 'TVSMOTOR': 20, 'PIDILITIND': 50,
            'CGPOWER': 10, 'ADANIGREEN': 20, 'MANKIND': 20, 'POLICYBZR': 20,
            'HDFCAMC': 20, 'ALKEM': 100, 'LUPIN': 10, 'GLENMARK': 20,
